...
def generate_images(
    ...
    subdirs=False,  # NOTE: Origianlly create subdir for every seed, here we create it for every class.
        ...
        def __iter__(self):
            ...
            for batch_idx, indices in enumerate(rank_batches):
                ...
                if len(r.seeds) > 0:
                    ...
                    r.noise = rnd.randn(
                        [len(r.seeds), net.img_channels, net.img_resolution, net.img_resolution], device=device
                    )
                    r.labels = None
                    if net.label_dim > 0:
                    ...
                    if outdir is not None:
                        ...
                        for seed, image, label in zip(
                            r.seeds, r.images.permute(0, 2, 3, 1).cpu().numpy(), r.labels.argmax(dim=1).cpu().numpy()
                        ):
                            label_dir = os.path.join(outdir, f"{label}") if subdirs else outdir
                            os.makedirs(label_dir, exist_ok=True)
                            PIL.Image.fromarray(image, "RGB").save(os.path.join(label_dir, f"{seed:06d}.png"))
...
@click.option(
    "--guidance", help="Guidance strength  [default: 1; no guidance]", metavar="FLOAT", type=float, default=None
)
@click.option(
    "--S_churn",
    "S_churn",
...
@click.option(
    "--S_noise", "S_noise", help="Stoch. noise inflation", metavar="FLOAT", type=float, default=1, show_default=True
)
