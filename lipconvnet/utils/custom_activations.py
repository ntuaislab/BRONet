...
class VReLU(nn.Module):
    def __init__(self):
        super(VReLU, self).__init__()

    def forward(
        self,
        z,
    ):
        zn = F.relu(z) + F.relu(-z)
        return zn
...
class HouseHolder(nn.Module):
    def __init__(self, channels):
        ...
        self.theta = nn.Parameter(0.5 * np.pi * torch.ones(1, eff_channels, 1, 1).cuda(), requires_grad=True)
...
class HouseHolder_Order_2(nn.Module):
    def __init__(self, channels):
        ...
        self.theta0 = nn.Parameter((np.pi * torch.rand(self.num_groups)).cuda(), requires_grad=True)
        self.theta1 = nn.Parameter((np.pi * torch.rand(self.num_groups)).cuda(), requires_grad=True)
        self.theta2 = nn.Parameter((np.pi * torch.rand(self.num_groups)).cuda(), requires_grad=True)
