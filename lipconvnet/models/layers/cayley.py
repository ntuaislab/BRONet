...
class Cayley(StridedConv, nn.Conv2d):
    def __init__(self, *args, **kwargs):
        super().__init__(*args, **kwargs)
        self.alpha = nn.Parameter(torch.ones(1, dtype=torch.float32, requires_grad=True).to(self.weight.device))
    ...
    def forward(self, input):
        cout, cin, k1, k2 = self.weight.shape  # cout, cin, kernel_size, kernel_size
        assert self.kernel_size[0] == self.kernel_size[1]
        padded_n = 0
        if k1 > 1:
            input = F.pad(input, (k1 // 2, k1 // 2, k2 // 2, k2 // 2))
            padded_n = padded_n + self.kernel_size[0] // 2
        batches, _, n, _ = input.shape  # batch, cin, n, n
        if not hasattr(self, 'shift_matrix'):
            s = (self.weight.shape[2] - 1) // 2
            self.shift_matrix = self.fft_shift_matrix(n, -s)[:, : (n // 2 + 1)].reshape(n * (n // 2 + 1), 1, 1).to(input.device)

        xfft = torch.fft.rfft2(input, s=input.shape[2:]).permute(2, 3, 1, 0).reshape(n * (n // 2 + 1), cin, batches)
        wfft = self.shift_matrix * torch.fft.rfft2(self.weight, (n, n)).reshape(cout, cin, n * (n // 2 + 1)).permute(2, 0, 1).conj()
        if self.alpha is None:
            self.alpha = nn.Parameter(torch.tensor(wfft.norm().item(), requires_grad=True).to(input.device))
        yfft = (cayley_func(self.alpha * wfft / wfft.norm()) @ xfft).reshape(n, n // 2 + 1, cout, batches)

        y = torch.fft.irfft2(yfft.permute(3, 2, 0, 1), input.shape[2:])
        if padded_n > 0:
            y = y[:, :, padded_n:-padded_n, padded_n:-padded_n]
...
class CayleyLinear(nn.Linear):
    ...
    def forward(self, input):
        ...
        if self.training or self.Q is None:
            self.Q = cayley_func(self.alpha * self.weight / self.weight.norm())
        return F.linear(input, self.Q if self.training else self.Q.detach(), self.bias)
...
class PlainConv(nn.Conv2d):
    def forward(self, input):
        ...
        if self.kernel_size[0] == 2:
            return super().forward(F.pad(input, (0, 1, 0, 1), mode="circular"))
        return super().forward(F.pad(input, (1, 1, 1, 1)))
